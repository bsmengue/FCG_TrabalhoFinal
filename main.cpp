...
int main(int argc, char* argv[])
{
    ...
    LoadTextureImage("../../data/rocky_terrain_02_diff_4k.jpg");
    ...
    ObjModel charizardmodel("../../data/charizard.obj");
    ComputeNormals(&charizardmodel);
    BuildTrianglesAndAddToVirtualScene(&charizardmodel);

    ObjModel bulbasaurmodel("../../data/bulbasaur.obj");
...
{
    if (!shape.name.empty())
        g_CarParts.push_back(shape.name);
}
if (g_CarParts.empty())
{
...
{
    if (!shape.name.empty())
        g_BulbasaurParts.push_back(shape.name);
}
if (g_BulbasaurParts.empty())
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        float delta_time = time_current - time_prev;
...
//------------------------------------------
// --- Colisão Carro 1 x Carro 2 (empurrão realista) ---
//------------------------------------------
...
else
{
    ...
    // Charizard
        ...
        if (!g_BulbasaurParts.empty()) {
            model = Matrix_Translate(g_Car2Position.x, g_Car2Position.y + 0.5f, g_Car2Position.z)
    ...
    if (!g_CarParts.empty()) {
        model = Matrix_Translate(g_Car2Position.x, g_Car2Position.y, g_Car2Position.z)
              * Matrix_Scale(1.2f, 1.2f, 1.2f);
        glUniformMatrix4fv(g_model_uniform, 1, GL_FALSE, glm::value_ptr(model));
        glUniform1i(g_object_id_uniform, CAR);
...
std::string FindObjectBySubstring(const std::string& token)
{
    ...
    std::transform(token_lower.begin(), token_lower.end(),
                   token_lower.begin(), ::tolower);

    for (const auto& kv : g_VirtualScene)
    {
        std::string lower = kv.first;
        std::transform(lower.begin(), lower.end(),
                       lower.begin(), ::tolower);

        if (lower.find(token_lower) != std::string::npos)
        {
            // Encontrou um objeto cujo nome contém o token
            return kv.first;
        }
    }
